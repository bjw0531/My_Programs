...
def switchLocks_ard(ser, Key):
    dicts = {VK_Capital: '0xC1',
             VK_ScrollLock: '0xCF'}

    time.sleep(0.01)
    ser.write(f'p{dicts.get(Key)}'.encode())
    print(f'p {dicts.get(Key)}')

    time.sleep(0.01)
    ser.write(f'r{dicts.get(Key)}'.encode())
    print(f'r {dicts.get(Key)}')
...
class runThread(QThread):
    ...
    def run(self):
        self.current_port = self.parent.ports.currentText()
        self.current_port = portNameSlice(self.current_port)

        try:
            self.ser
        except:
            try:
                self.ser = serial.Serial(self.current_port, 115200)
            except:
                return
            ...
            if self.line[0] == 'p':
                try:
                    ...
                    if self.tmp not in self.set:
                        ...
                        print(f'p {self.tmp}')
                        self.ser.write(f'p{self.tmp}'.encode())
                ...
                try:
                    ...
                    self.ser.write(f'r{self.tmp}'.encode())
        ...
        if getLockState(VK_Capital):
            # switchLocks(VK_Capital)
            switchLocks_ard(self.ser, VK_Capital)
...
class MyWindow(QMainWindow, form_class):
    def __init__(self):
        ...
        self.refresh.clicked.connect(self.portRefresh)
        self.portRefresh()
    ...
    def portRefresh(self):
        self.ports.clear()
        self.portlist = sp.comports()
        self.connected = []
        for port, desc, hwid in sorted(self.portlist):
            self.ports.addItem(f"{port}: {desc}")
